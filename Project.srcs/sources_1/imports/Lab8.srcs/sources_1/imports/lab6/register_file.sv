...
module register_file #(
   ...
   input wire clk,
   input wire wr,                            // WriteEnable:  if wr==1, data is written into me
   ...
   logic [Dbits-1:0] rf [Nloc-1:0];                     // The actual registers where data is stored                                         // initial $readmemh(initfile, ..., ..., ...);  
   ...
   assign ReadData1 = (ReadAddr1 == 0) ? 32'b0 : rf[ReadAddr1];     // First output port
   assign ReadData2 = (ReadAddr2 == 0) ? 32'b0 : rf[ReadAddr2];     // Second output port
